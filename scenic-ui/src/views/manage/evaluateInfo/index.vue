    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['manage:evaluateInfo:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['manage:evaluateInfo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['manage:evaluateInfo:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList" :columns="columns"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="evaluateInfoList" @selection-change="handleSelectionChange">
...
<script>
import { listEvaluateInfo, getEvaluateInfo, delEvaluateInfo, addEvaluateInfo, updateEvaluateInfo } from "@/api/manage/evaluateInfo";
...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      // 导出地址
      exportUrl: 'manage/evaluateInfo/export',
      // 表单校验
      rules: {
        scenicId: [
  ...
  methods: {
    ...
    getList() {
      ...
      listEvaluateInfo(this.queryParams).then(response => {
        this.evaluateInfoList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getEvaluateInfo(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateEvaluateInfo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addEvaluateInfo(this.form).then(response => {
    ...
    handleExport() {
      this.download(this.exportUrl, {
        ...this.queryParams
      }, `evaluateInfo_${new Date().getTime()}.xlsx`)
    },
  }
};
</script>
