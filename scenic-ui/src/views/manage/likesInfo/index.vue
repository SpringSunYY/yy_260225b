...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      // 导出地址
      exportUrl: 'manage/likesInfo/export',
      // 表单校验
      rules: {
        scenicId: [
  ...
  methods: {
    ...
    getList() {
      ...
      listLikesInfo(this.queryParams).then(response => {
        this.likesInfoList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateLikesInfo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addLikesInfo(this.form).then(response => {
    ...
    handleExport() {
      this.download(this.exportUrl, {
        ...this.queryParams
      }, `likesInfo_${new Date().getTime()}.xlsx`)
    },
  }
};
</script>
