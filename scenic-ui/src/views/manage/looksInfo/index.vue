<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['manage:looksInfo:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['manage:looksInfo:edit']"
        >修改
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['manage:looksInfo:remove']"
        >删除
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['manage:looksInfo:export']"
        >导出
        </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="looksInfoList" @selection-change="handleSelectionChange">
...
<script>
import {addLooksInfo, delLooksInfo, getLooksInfo, listLooksInfo, updateLooksInfo} from "@/api/manage/looksInfo";
...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      // 导出地址
      exportUrl: 'manage/looksInfo/export',
      // 表单校验
      rules: {
        scenicId: [
  ...
  methods: {
    ...
    getList() {
      ...
      listLooksInfo(this.queryParams).then(response => {
        this.looksInfoList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateLooksInfo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addLooksInfo(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除浏览信息编号为"' + ids + '"的数据项？').then(function () {
        return delLooksInfo(ids);
      }).then(() => {
        this.getList();
    ...
    handleExport() {
      this.download(this.exportUrl, {
        ...this.queryParams
      }, `looksInfo_${new Date().getTime()}.xlsx`)
    },
  }
};
</script>
